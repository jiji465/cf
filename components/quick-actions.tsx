...
import { saveObligation } from "@/lib/supabase/database"
...
export function QuickActions({ obligations, onUpdate }: QuickActionsProps) {
  ...
  const handleBulkComplete = async (obligationList: ObligationWithDetails[]) => {
    if (confirm(`Tem certeza que deseja marcar ${obligationList.length} obrigação(ões) como concluída(s)?`)) {
      try {
        ...
        console.error("[v0] Error completing obligations:", error)
        alert("Erro ao concluir obrigações. Tente novamente.")
      }
    }
  }

  const handleBulkStart = async (obligationList: ObligationWithDetails[]) => {
    if (confirm(`Tem certeza que deseja iniciar ${obligationList.length} obrigação(ões)?`)) {
      try {
        ...
        console.error("[v0] Error starting obligations:", error)
        alert("Erro ao iniciar obrigações. Tente novamente.")
      }
    }
  }
